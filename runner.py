...
def get_arguments(arg_list=None):
    ...
    parser.add_argument(
        ...
        nargs=2,
        default=None,
        help="Points are only sampled within a bounded distance from nuclei [Å]",
    )
    parser.add_argument(
        "--split_file",
        type=str,
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        collate_fn=dataset.CollateFuncRandomSample(args.cutoff, 1000, pin_memory=False, set_pbc_to=set_pbc, sampling_bounds=args.sampling_bounds),
    )
    val_loader = torch.utils.data.DataLoader(
        datasplits["validation"],
        2,
        collate_fn=dataset.CollateFuncRandomSample(args.cutoff, 5000, pin_memory=False, set_pbc_to=set_pbc, sampling_bounds=args.sampling_bounds),
        num_workers=0,
    )
